...
<%
   ...
   ArrayList<Mypage> mList = (ArrayList<Mypage>) request.getAttribute("mList");
...
<script>
   $(function() {
      ...
      $a.append("<div style='height: 5px; background:white;'></div>");
   });
   $(function() {
...
<div class="mypage-contains">
   <div class="mypage-wrap">
      <div class="mypage-seg">
         ...
         <div class="mypage-profile">
            ...
            <% if (logCount == 0) { %>
               ...
               <span class="badge badge-pill badge-danger new-news" style="background: grey;"><%=logCount %></span>
            <% } else if (logCount > 0 ) { %>
               <a class="news-btn" href="#"><span class="glyphicon glyphicon-bell" style="color : red;"></span></a>
               <span class="badge badge-pill badge-danger new-news"><%=logCount %></span>
            <% } %>
            </p>
      ...
      <div class="mypage-portfolio-wrap">
         <button type="button" class="portfolio_btn btn btn-default btn-md" id="userModify-btn">내 정보변경</button>
         <br />
         <br />
         <button type="button" class="portfolio_btn btn btn-primary btn-md" onclick="portfolio_open();">포트폴리오</button>
      </div>
   </div>
   <div class="mypage-content-btn">
      ...
      <% } %>
      <% } else if ( index == 1 ) { for ( Mypage mp : mList ) { %>
      <!--  내가 쓴 댓글 -->
      <div class="mypage-content-body-wrap">
         <div class="mypage-content-body">
            <table>
      ...
      <% } } else if ( index == 2 ) { for ( Mypage mp : mList ) {  %>
      <!--  내 글에 답글 -->
      <div class="mypage-content-body-wrap">
         <div class="mypage-content-body">
            <table>
               <tbody>
                  <tr class="mypage-table-line">
                     ...
                     <td class="line-day"><%=mp.getBoard_elapsed_date() %>일</td>
                     <td>전</td>
                     <% if (mp.getLog_check() == 0) { %>
                     <td class="line-board" style="color: blue;">
                        <a href="<%=request.getContextPath()%>/board_detail.cp?index=<%=mp.getBoard_index()%>&rindex=<%=mp.getReply_index()%>&logCheck=<%=mp.getLog_check()%>">
                           <%=mp.getReply_content() %>
                        </a>
                     </td>
                     <% } else { %>
                     <td class="line-board" style="color: gray;">
                        <a id="logCheckbtn" href="<%=request.getContextPath()%>/board_detail.cp?index=<%=mp.getBoard_index()%>&rindex=<%=mp.getReply_index()%>&logCheck=<%=mp.getLog_check()%>">
                           <%=mp.getReply_content() %>
                        </a>
                     </td>
                     <% } %>
